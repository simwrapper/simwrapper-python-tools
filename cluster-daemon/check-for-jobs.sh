...
# 2. If it finds any new jobs in queue, it calls squeue-launcher.py which *writes* squeue-launch.sh which
#    runs the command via squeue.
# 3. "check_for_jobs status" is also run from cron every three minutes (but one minute later) and
#    and it calls the squeue status command to see if any jobs are running
...
check_for_new_jobs() {
  ...
  if [ $JOBSIZE -gt 5 ]; then
    APIKEY="$APIKEY" python3 ~/squeue-launcher.py
  fi
}
...
check_for_running_jobs() {
  ...
  if [ $JOBSIZE -gt 5 ]; then
     APIKEY="$APIKEY" python3 ~/squeue-launcher.py check_status
  fi
}
